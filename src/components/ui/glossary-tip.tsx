...
export function GlossaryTip({ term, children }: GlossaryTipProps) {
  ...
  return (
    <span ref={ref} className="relative inline-flex items-center gap-1">
      ...
      {open && (
        <span className="absolute bottom-full left-0 mb-2 z-50 w-64 bg-[var(--sidebar)] border border-border rounded-xl p-3 shadow-2xl text-xs text-muted-foreground leading-relaxed pointer-events-none">
...
export function InfoTip({ children }: InfoTipProps) {
  ...
  return (
    <span ref={ref} className="relative inline-flex">
      ...
      {open && (
        <span className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 z-50 w-72 bg-[var(--sidebar)] border border-border rounded-xl p-3 shadow-2xl text-xs text-muted-foreground leading-relaxed pointer-events-none">
          {children}
