...
import { Heart, Utensils, FlaskConical, Stethoscope } from 'lucide-react'
...
export function SaudeNutricaoTab({ athleteId, sex }: { athleteId: string; sex: 'M' | 'F' | null }) {
  const [sub, setSub] = useState<'saude' | 'clinico' | 'exames' | 'nutricao'>('saude')
  return (
    <div className="space-y-4">
      <div className="flex gap-1 p-1 rounded-xl bg-secondary w-full sm:w-fit">
        {([['saude', 'Saúde', Heart], ['clinico', 'Clínico', Stethoscope], ['exames', 'Exames', FlaskConical], ['nutricao', 'Nutrição', Utensils]] as const).map(([k, label, Icon]) => (
