...
export function SaudeNutricaoTab({ athleteId, sex }: { athleteId: string; sex: 'M' | 'F' | null }) {
  const [sub, setSub] = useState<'saude' | 'clinico' | 'exames' | 'nutricao' | 'medidas'>('saude')
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap gap-1 p-1 rounded-xl bg-secondary w-full sm:w-fit">
        {([['saude', 'Saúde', Heart], ['clinico', 'Clínico', Stethoscope], ['exames', 'Exames', FlaskConical], ['nutricao', 'Nutrição', Utensils], ['medidas', 'Medidas', Scale]] as const).map(([k, label, Icon]) => (
      ...
      {sub === 'clinico' && <ClinicalPanel athleteId={athleteId} sex={sex} />}
      {sub === 'nutricao' && <NutricaoTab athleteId={athleteId} />}
      {sub === 'medidas' && <MetricasTab athleteId={athleteId} />}
